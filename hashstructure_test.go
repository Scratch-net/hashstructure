...
func TestHash_identity(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if bytes.Compare(valuelist[0], zeroHash) == 0 {
		...
		for i := 1; i < len(valuelist); i++ {
			if bytes.Compare(valuelist[i], valuelist[0]) != 0 {
...
func TestHash_equal(t *testing.T) {
	...
	for i, tc := range cases {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			// Zero is always wrong
			if bytes.Compare(one, zeroHash) == 0 {
				t.Fatalf("zero hash: %#v", tc.One)
			}

			// Compare
			if (bytes.Compare(one, two) == 0) != tc.Match {
...
func TestHash_ignoreZeroValue(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if (bytes.Compare(hashA, hashB) == 0) != tc.IgnoreZeroValue {
